...
impl<'s> System<'s> for Movement {
    ...
    fn run(&mut self, (players, cameras, mut transforms): Self::SystemData) {
        let mut player_translation = Vector2 {x: 0.0 as f32, y: 0.0 as f32,};
        ...
        for (_, transform) in (&cameras, &mut transforms).join() {
            ...
            let camera_safe_edge = player_direction.normalize_to((transform.scale.truncate() / 4 as f32).magnitude());
